...
((root)=>{
  ...
  function create(storage){let data,loadError=null;
    ...
    return {get:()=>clone(data),loadError,stats,restStats,restSessions,members,allSessions,dateKey,periodRange,questProgress,
      ...
      comment(postId,content){postFor(postId);content=content.trim();if(!content||content.length>500)throw Error('댓글을 1~500자로 적어주세요.');commit(next=>next.comments.push({id:id(),postId,content,author:'me',createdAt:Date.now()}));},
