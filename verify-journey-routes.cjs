...
(async () => {
  ...
  try {
    for (const width of [1440,768,375,320]) {
      ...
      check(loop.focus.scene.includes('낚시섬')&&loop.focus.scene.includes('카운트업')&&loop.focus.action.includes('1분마다 물고기 1마리'),'집중: 낚시섬 자유 자리·카운트업·1분 1마리');
      check(loop.rest.scene.includes('식빵')&&loop.rest.action.includes('restSeat')&&loop.rest.next.includes('같은 자리·같은 세션'),'휴식: 전용 모닥불·자동 자리·같은 세션 재개');
