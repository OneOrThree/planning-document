...
(async()=>{
 ...
 try{
  for(const width of [1440,1024,768,414,375,320]){
   ...
   check(await page.locator('.growth-cost-table .is-open').count()===0&&await page.locator('.growth-cost-table tbody tr:not([data-cost=start]) td:last-child',{hasText:'초기값 채택'}).count()===7,'건설 수치는 초기값 채택 표시');
   ...
   check(!/마을 포인트|\d+P\b|돛단배|개인 물고기|배 위 낚시/.test(growthText)&&growthText.includes('총액')&&growthText.includes('각자 몫')&&!growthText.includes('1인당 요구량')&&growthText.includes('완료 도장'),'성장 탭 옛 재화 문구 제거·총액·각자 몫 차감과 완료 도장 규칙');
   ...
   check(entries.length===37,'결정 로그 37건');
   for(const id of ['policy-fish-currency','policy-building-quest','policy-daily-quest-reward','policy-focus-fishing-island','policy-library','policy-join-capacity','policy-observatory-ranking','policy-shop-anyone','policy-art-guides','policy-friends','policy-mailbox-friend-chat'])check(entries.find(e=>e.id===id)?.status==='confirmed','현재 정책 확정 로그 '+id);
