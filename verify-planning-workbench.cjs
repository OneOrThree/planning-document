...
(async()=>{
 ...
 try{
  for(const width of [1440,1024,768,414,375,320]){
   ...
   check(gramophoneCost.includes('1인당 50마리')&&gramophoneCost.includes('5분'),'축음기 1인당 50마리·공사 5분');
   check(shopCost.includes('다른 모든 건물')&&shopCost.includes('1인당 250마리')&&shopCost.includes('1시간'),'상점은 마지막·1인당 250마리·공사 1시간');
   const growthText=await page.locator('#growth-view').textContent();
   check(!/마을 포인트|\d+P\b|돛단배|개인 물고기|배 위 낚시/.test(growthText)&&growthText.includes('대상 인원 × 1인당 요구량')&&growthText.includes('완료 도장'),'성장 탭 옛 재화 문구 제거·차감과 완료 도장 규칙');
