...
(async()=>{
 ...
 try{
  for(const width of [1440,1024,768,414,375,320]){
   ...
   check(entries.length===43,'결정 로그 43건');
   for(const id of ['policy-user-blocking','policy-golden-fish','focus-emote-symbol-stickers','policy-fish-currency','policy-building-quest','policy-daily-quest-reward','policy-focus-fishing-island','policy-library','policy-join-capacity','policy-observatory-ranking','policy-shop-anyone','policy-art-guides','policy-friends','policy-mailbox-friend-chat'])check(entries.find(e=>e.id===id)?.status==='confirmed','현재 정책 확정 로그 '+id);
