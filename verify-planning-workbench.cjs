...
(async()=>{
 ...
 try{
  for(const width of [1440,1024,768,414,375,320]){
   ...
   check(await page.locator('.growth-cost-table .is-open').count()===0&&await page.locator('.growth-cost-table tbody tr:not([data-cost=start]) td:last-child',{hasText:'확정'}).count()===7,'건설 수치는 현재 정책 확정 표시');
   ...
   check(gramophoneCost.includes('총액 1,360마리')&&gramophoneCost.includes('30분'),'축음기 총액 1,360마리·공사 30분');
   check(shopCost.includes('다른 모든 건물')&&shopCost.includes('총액 6,800마리')&&shopCost.includes('4시간'),'상점은 마지막·총액 6,800마리·공사 4시간');
   const growthText=await page.locator('#growth-view').textContent();
   check(!/마을 포인트|\d+P\b|돛단배|개인 물고기|배 위 낚시/.test(growthText)&&growthText.includes('총액 ÷ 대상 인원')&&growthText.includes('완료 도장'),'성장 탭 옛 재화 문구 제거·고정 총액과 완료 도장 규칙');
   ...
   check((await page.locator('.journey-return').textContent()).includes('같은 자리·같은 세션')&&(await page.locator('.journey-return').textContent()).includes('섬 화면'),'휴식 뒤 같은 자리 재개·종료 뒤 섬 화면');
   const mailboxBranch=await page.locator('[data-growth-branch=mailbox]').textContent();
   check(mailboxBranch.includes('친구 편지는 비동기')&&mailboxBranch.includes('읽고 닫으면')&&mailboxBranch.includes('우체통이 없어도'),'우체통: 섬 편지방 + 휘발성 친구 편지');
   ...
   check(entries.length===41,'결정 로그 41건');
   for(const id of ['policy-solo-owner-delete','policy-hall-modal','policy-rest-screen','policy-friend-letter','policy-main-island','policy-auth-guest','policy-fish-currency','policy-building-quest','policy-daily-quest-reward','policy-focus-fishing-island','policy-library','policy-join-capacity','policy-observatory-ranking','policy-shop-anyone','policy-art-guides'])check(entries.find(e=>e.id===id)?.status==='confirmed','현재 정책 확정 로그 '+id);
   for(const id of ['focus-rest','growth','decoration','music','open-policy'])check(entries.find(e=>e.id===id)?.status==='superseded','현재 정책으로 대체한 이전안 '+id);
