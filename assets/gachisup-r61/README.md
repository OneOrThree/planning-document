...
### 2026-09-20 앱 사용 그림 추가

앱(`phone` 저장소 `app/app-dev/src/assets/`) 화면이 실제로 쓰는데 여기 없던 그림 87개를 앱 경로 그대로 추가했다. 첫 화면은 402개(PNG 363, JPG 29, SVG 4, WAV 3, TTF 3)다.

- 추가: `interiors/` 24개(건물 실내 배경·아바타·게시판/우체통 UI), `reference-v2/` 55개(앱 화면의 건물 아이콘·건물 배경·아바타·이모티콘·섬 전체·첫 섬 등), 고양이 6색 식빵 굽기 자세, 휴식 모닥불 배경, 낚시섬 바다 타일.
...
- 앱 코드에서 불리지 않는 파일(밤 배경 레이어, 돛단배·선실 배, 번호 붙은 옛 배경 등)은 넣지 않았다.

## 첫 화면 전체 에셋 (402개)

| 폴더 | 개수 | 내용 |
| --- | --- | --- |
| `characters/` | 230 | 고양이 6색의 자세·프레임·아틀라스·식빵 굽기, 몽돌·강아지·앵무새·팰리컨 NPC |
