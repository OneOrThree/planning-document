...
const updates = {
  ...
  'fn-settings': {
    purpose: '앱·알림·소리 설정과 내가 차단한 사용자 목록을 관리한다. 차단 목록에서 해제하면 서버 정본을 다시 조회한다.',
  },
  'FR-1': {
    purpose: '내 뗏목에서 친구 검색·요청·수락·거절·요청 취소·친구 삭제를 관리한다. 친구 프로필에서는 확인 단계를 거쳐 사용자를 차단할 수 있다.',
  },
  'FR-4': {
    purpose: '친구 목록과 친구 프로필에서 메인 섬 이름을 확인하고 관계를 삭제한다. 친구 프로필의 차단은 확인 뒤 실행하고 성공하면 친구 목록을 다시 조회한다.',
  },
  'fn-field-28': {
  ...
  'SH-2': {
    purpose: '대상과 강퇴 결과를 명확히 확인한다. 초기 운영과 회관에서 같은 시트를 재사용한다. 대상의 메인 섬이면 남은 섬 중 가장 최근에 가입한 섬이 메인 섬이 되고 알림을 보낸다. 강퇴로 소속 섬이 하나도 남지 않은 사용자도 첫 소속 선택으로 간다.',
  },
  'SYS-6': {
    purpose: '탈퇴·강퇴·섬 삭제 뒤 접근할 수 없는 홈에서 빠져나와 남은 소속 섬이나 첫 소속 선택으로 연결한다. 마지막 소속 섬에서 탈퇴하거나 혼자 남은 방장이 섬을 삭제해 소속이 없어지면 첫 소속 선택으로 간다. 강퇴로 소속 섬이 하나도 남지 않은 사용자도 첫 소속 선택으로 간다. 기존 자산과 과거 기록은 별도로 보존한다.',
  ...
  'PO-3': {
    purpose: '내게 도착한 친구 편지를 시간순으로 확인한다. 발신자·보낸 시각을 표시하고 편지를 누르면 상세 내용을 연다. 받은 편지 상세에서는 확인 단계를 거쳐 발신자를 차단할 수 있다. 연 편지는 닫으면 양쪽 목록에서 지운다. 내가 속한 섬 중 우체통이 완공된 섬에서 확인한다.',
  },
  'fn-friend-read': {
    purpose: '선택한 편지의 발신자·수신자·보낸 시각·본문을 확인한다. 받은 편지에서는 확인 단계를 거쳐 발신자를 차단하고 성공하면 받은 편지 목록을 다시 조회한다.',
  },
