...
const updates = {
  ...
  'fn-island-manage': {
    purpose: '섬 등록증에서 섬 이름·소개·가입 방식·정원과 가입 신청을, 주민 화면에서 멤버 관리·방장 위임·섬 탈퇴를 다룬다. 주민은 읽기와 본인 탈퇴만 한다. 일반 주민은 마지막 소속 섬에서 탈퇴할 수 없지만 혼자 남은 방장은 섬 삭제 확인 뒤 섬을 삭제할 수 있다.',
  },
  'fn-field-28': {
    purpose: '방장 위임·섬 탈퇴 — 섬 정보·운영의 세부 기능. 일반 주민은 마지막 소속 섬에서 탈퇴할 수 없다. 혼자 남은 방장은 삭제 범위와 다음 도착지를 확인하고 섬을 삭제한다. 다른 소속이 있으면 남은 메인 섬으로, 마지막 소속이었다면 첫 섬 선택으로 이동한다.',
  },
  'SYS-6': {
    purpose: '탈퇴·강퇴·섬 삭제 뒤 접근할 수 없는 홈에서 빠져나와 남은 소속 섬이나 첫 소속 선택으로 연결한다. 일반 주민은 마지막 소속 섬에서 탈퇴할 수 없고, 혼자 남은 방장은 섬을 삭제해 소속 없는 상태로 돌아갈 수 있다. 마지막 섬에서 강퇴된 사용자의 처리는 추가 결정이 필요하다. 기존 자산과 과거 기록은 별도로 보존한다.',
