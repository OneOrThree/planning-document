...
(async()=>{
  ...
  const label=value('label',''),phase=value('phase','prepare'),from=Number(value('from',.28)),to=Number(value('to',.95)),selected=value('film',null),samples=Number(value('samples',6)),cropSize=Number(value('crop',360));
  ...
  try{
    ...
    const data=await page.evaluate(({phase,from,to,selected,samples,cropSize})=>{
      ...
      return CutsceneProduction.films.filter(f=>!selected||f.id===selected).map(f=>{
        ...
        for(let i=0;i<samples;i++){
          ...
          const crop={x:Math.max(0,Math.min(720-cropSize,sx-cropSize*.378)),y:Math.max(0,Math.min(1280-cropSize,sy-cropSize*.778)),size:cropSize};
    ...
    },{phase,from,to,selected,samples,cropSize});
    ...
    fs.writeFileSync(path.join(out,'frames.json'),JSON.stringify({label,phase,from,to,samples,cropSize,fingerprint:hash.digest('hex'),errors,films:data},null,2)+'\n');
