...
async function main(){
  ...
  const out=path.join(root,'output','cutscenes','reviews',label);fs.mkdirSync(out,{recursive:true});
  ...
  try{
    ...
    const data=await page.evaluate(({phase,actor})=>{
      ...
      for(let d=0;d<3;d++){
        ...
        CutsceneProduction.films.filter(f=>f.directionIndex===d).forEach((f,row)=>{
          ...
          const k=f.timing;const range={prepare:[.8,k.prepareEnd-.3],walk:[k.prepareEnd+.15,k.walkEnd-.05],boarding:[k.walkEnd+.1,k.boardingEnd+.15],settle:[k.boardingEnd+.01,k.departureStart],depart:[k.departureStart,k.seaStart-.1],sea:[k.seaStart+.3,23.8]}[phase];if(!range)throw Error('알 수 없는 검토 구간');
    ...
    for(const f of ['cutscene-cinema.js','cutscene-renderer.js','cutscene-production.js','cutscene-sprites.js','cutscene-rig.js'])hash.update(f).update(fs.readFileSync(path.join(root,f)));
