...
const exportCatalog = require('../cutscene-export-catalog.js');
...
let excludedReviewImages=0,excludedDiagnosticScreenshots=0,archivedVideos=0;
const archivedVersions=new Set(exportCatalog.versions.slice(exportCatalog.inlineCount));
const publishFile=src=>{
  ...
  const screenshotName=path.basename(relative);
  if(relative.startsWith('screenshots/')&&(/^(FAIL-|EDGE-)/i.test(screenshotName)||/(before|debug)/i.test(screenshotName))){excludedDiagnosticScreenshots++;return false;}
  const movie=relative.match(/^output\/cutscenes\/videos\/([^/]+)\/[^/]+\.mp4$/i);
  if(movie&&archivedVersions.has(movie[1])){archivedVideos++;return false;}
  return true;
};
...
console.log(JSON.stringify({publishedBytes:bytes,excludedReviewImages,excludedDiagnosticScreenshots,archivedVideos,reviewOriginals:'Git 저장소에 보존'}));
