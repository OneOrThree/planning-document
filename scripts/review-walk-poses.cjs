...
(async()=>{
  ...
  if(!['walk','boarding','settle','recovery','paddle','blink'].includes(phase))throw Error('비교 구간은 walk / boarding / settle / recovery / paddle / blink입니다.');
  ...
  try{
    ...
    const duration=await page.evaluate(({filmId,phase,paddle,walk,single})=>{
      ...
      const [start,end]={walk:[k.prepareEnd,k.walkEnd],boarding:[k.walkEnd-.25,k.boardingEnd+.45],settle:[k.boardingEnd,k.departureStart],recovery:[k.boardingEnd+1.15,k.departureStart+.2],paddle:[k.departureStart+.9,k.departureStart+3.9],blink:[blinkAt-.1,blinkAt+.4]}[phase];
    ...
    },{filmId,phase,paddle,walk,single});
    ...
    const hash=crypto.createHash('sha256');for(const file of ['cutscene-cinema.js','cutscene-renderer.js','cutscene-poses.js','cutscene-paddle-rig.js','cutscene-walk-rig.js','cutscene-sprites.js','cutscene-production.js'])hash.update(file).update(fs.readFileSync(path.join(root,file)));
    ...
    fs.writeFileSync(path.join(out,'frames.json'),JSON.stringify({filmId,phase,single,duration:frames/fps,fps,frames,fingerprint:hash.digest('hex'),status:'unreviewed-experiment',note:'실제 속도의 선택 구간. 전체 컷신이나 동작의 자동 통과 판정이 아니다. 연락표는 24개 시간 표본.',errors,states},null,2)+'\n');
