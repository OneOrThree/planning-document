...
(async()=>{
  ...
  const label=value('label',''),filmId=value('film','new-morning-emotion'),phase=value('phase','walk'),paddle=value('paddle',''),walk=value('walk','');
  ...
  try{
    ...
    const duration=await page.evaluate(({filmId,phase,paddle,walk})=>{
      ...
      window.walkPoseFrame=(elapsed,sample,frameIndex)=>{
        ...
        for(const [i,mode] of (paddle==='rig'||walk==='rig'?['poses','poses']:['sprite','poses']).entries()){
          const f={...film,tuning:{...film.tuning,actorMode:mode,paddleRig:paddle==='rig'?i===1:film.tuning.paddleRig,walkRig:walk==='rig'&&i===1}},m=GachisupCinema.render(source,f,time),cam=m.camera;
          ...
          c.drawImage(source,crop.x*2,crop.y*2,560,560,12+i*420,48,396,396);c.fillStyle='#3c493c';c.font='600 17px system-ui';c.fillText(walk==='rig'?(i?'발 접지 관절 · 실험':'개별 8자세 · 기준'):paddle==='rig'?(i?'앞발 국소 관절 · 실험':'고정 앞발 · 기준'):(i?'개별 자세 · 실험':'기존 아틀라스 · 기준'),15+i*420,29);
