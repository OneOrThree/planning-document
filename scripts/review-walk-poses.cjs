...
(async()=>{
  ...
  if(setting&&!['raftContact','paddleWeight','clothFold','dockRig','tailRig','seabirds','oarTravel','raftReflection','readingBlink'].includes(setting))throw Error('없는 비교 설정');
  if(!Number.isFinite(cropSize)||cropSize<180||cropSize>720||!['cat','water','sky'].includes(anchor))throw Error('crop은 180~720, anchor는 cat / water / sky입니다.');
  if(!['packing','walk','walk-blink','reading-blink','dock','boarding','settle','recovery','paddle','blink'].includes(phase))throw Error('없는 비교 구간');
  ...
  try{
    ...
    if(water==='compare'&&!await page.evaluate(()=>CutsceneActors.waterMotionIntegrated)){
      await page.addScriptTag({path:path.join(root,'cutscene-water-motion.js')});
      await page.evaluate(()=>{const background=CutsceneActors.background;CutsceneActors.background=(c,img,t)=>{background(c,img,t);if(!CutsceneSettings.waterMotion)return;const scene=img===CutsceneActors.images.coastHome?'home':img===CutsceneActors.images.shore?'shore':null;if(scene)CutsceneWaterMotion.draw(c,scene==='home'?CutsceneActors.images.homeExtended:CutsceneActors.images.shoreExtended,scene,t);};});
    }
    const duration=await page.evaluate(({filmId,phase,paddle,walk,single,cropSize,anchor,water,setting})=>{
      ...
      const [start,end]={packing:[k.prepareEnd*.44,k.prepareEnd],walk:[k.prepareEnd,k.walkEnd],'walk-blink':[k.prepareEnd+.8,k.prepareEnd+1.3],'reading-blink':[k.prepareEnd*.36-.12,k.prepareEnd*.36+.38],dock:[k.walkEnd-1.5,k.walkEnd+.1],boarding:[k.walkEnd-.25,k.boardingEnd+.45],settle:[k.boardingEnd,k.departureStart],recovery:[k.boardingEnd+1.15,k.departureStart+.2],paddle:[k.departureStart+.9,k.departureStart+3.9],blink:[blinkAt-.1,blinkAt+.4]}[phase];
      ...
      window.walkPoseFrame=(elapsed,sample,frameIndex)=>{
        ...
        for(const [i,mode] of (single?['poses']:paddle==='rig'||walk==='rig'||water==='compare'||setting?['poses','poses']:['sprite','poses']).entries()){
          ...
          const focus=anchor==='sky'?CutsceneActors.seabirdsAt(time,f.directionId==='journey'?'home':'shore')[0]:m.catFoot;
          const x=360+(focus.x-cam.x)*cam.z,y=640+(focus.y-cam.y)*cam.z;
          const crop={x:Math.max(0,Math.min(720-cropSize,x-cropSize*(anchor==='water'?.38:120/280))),y:Math.max(0,Math.min(1280-cropSize,y-cropSize*(anchor==='sky'?.3:anchor==='water'?.48:218/280))),size:cropSize};
          c.drawImage(source,crop.x*2,crop.y*2,cropSize*2,cropSize*2,12+i*420,48,396,396);c.fillStyle='#3c493c';c.font='600 17px system-ui';c.fillText(setting?(setting==='raftContact'?'뗏목 수면 접촉':setting==='clothFold'?'덮개 접기':setting==='dockRig'?'부두 앞발 관절':setting==='tailRig'?'꼬리 국소 관절':setting==='seabirds'?'먼 하늘의 갈매기':setting==='raftReflection'?'수면에 비치는 뗏목':setting==='readingBlink'?'책 보는 눈 깜빡임':setting==='oarTravel'?'노와 전진의 박자':'상체 체중 이동')+(i||single?' · 수정':' · 기준'):single?'현재 시연 · '+phase:water==='compare'?(i?'수면 굴절 · 실험':'고정 수면 · 기준'):walk==='rig'?(i?'발 접지 관절 · 실험':'개별 8자세 · 기준'):paddle==='rig'?(i?'앞발 국소 관절 · 실험':'고정 앞발 · 기준'):(i?'개별 자세 · 실험':'기존 아틀라스 · 기준'),15+i*420,29);
        ...
        if(sample>=0)sc.drawImage(output,0,0,output.width,480,(sample%3)*tileWidth,Math.floor(sample/3)*240,tileWidth,240);
        const group=Math.floor(frameIndex/12),slot=frameIndex%12;
        if(!adjacent[group]){const tile=document.createElement('canvas');tile.width=tileWidth*3;tile.height=960;adjacent[group]=tile;}
        adjacent[group].getContext('2d').drawImage(output,0,0,output.width,480,(slot%3)*tileWidth,Math.floor(slot/3)*240,tileWidth,240);
    ...
    },{filmId,phase,paddle,walk,single,cropSize,anchor,water,setting});
    ...
    ff=spawn('ffmpeg',['-hide_banner','-loglevel','error','-y','-f','image2pipe','-vcodec','mjpeg','-framerate',String(fps),'-i','pipe:0','-an',...videoProfile.args,'-c:v','libx264','-crf','18','-pix_fmt','yuv420p','-movflags','+faststart',dest]);
    ...
    const hash=crypto.createHash('sha256');for(const file of ['cutscene-cinema.js','cutscene-renderer.js','cutscene-water-motion.js','cutscene-fur-palette.js','cutscene-poses.js','cutscene-paddle-rig.js','cutscene-walk-rig.js','cutscene-sprites.js','cutscene-production.js'])hash.update(file).update(fs.readFileSync(path.join(root,file)));
    ...
    fs.writeFileSync(path.join(out,'frames.json'),JSON.stringify({filmId,phase,single,water:water||null,setting:setting||null,cropSize,anchor,duration:frames/fps,fps,frames,videoProfile,fingerprint:hash.digest('hex'),status:'unreviewed-experiment',note:'실제 속도의 선택 구간. 전체 컷신이나 동작의 자동 통과 판정이 아니다. 연락표는 24개 시간 표본.',errors,states},null,2)+'\n');
