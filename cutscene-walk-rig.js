...
(() => {
  ...
  const ready=Promise.all(Object.entries({body:'walk-body-base-v2.png',leg:'walk-front-paw-layer-v1.png'}).map(([key,file])=>new Promise((resolve,reject)=>{const im=new Image();im.onload=()=>{images[key]=im;resolve();};im.onerror=()=>reject(Error('보행 레이어 로드 실패: '+file));im.src=base+file;})));
  ...
  function targets(x,y,size,options={}){
    const registration=.300488*size/384,stride=options.rigStride||size*.27,travel=options.travel||0,amount=clamp(options.walk??1),sample=options.pathAtDistance||((d)=>({x:x+d-travel,y}));
    return legs.map(leg=>{
      ...
      const contactDistance=(step-leg.phase+.31)*stride,a=sample(contactDistance),b=sample(contactDistance+stride),offset=(leg.sourceX-670.218)*registration,toe=leg.toeY*registration;
  ...
  function draw(c,x,y,size,time,options={}){
    ...
    function drawLeg(leg){
      ...
      c.save();c.translate(leg.foot.x,leg.foot.y);c.rotate(angle);c.scale(registration*leg.scale,registration*leg.scale);c.drawImage(images.leg,-646,-1126);c.restore();
      leg.angle=angle;
    }
    ...
    c.save();c.beginPath();c.rect(x-size,y-size*.18,size*2,size);c.clip();feet.filter(f=>f.id!=='far-front').forEach(drawLeg);c.restore();
