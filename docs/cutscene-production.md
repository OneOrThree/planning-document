...
`cutscenes.html`에서 9편을 선택하고 실시간 연출과 MP4 초안을 볼 수 있다. `draft-v09`부터 `draft-v01`까지 아홉 버전, 각각 9개 무음 MP4·포스터와 소스·에셋 지문을 가진 `manifest.json`이 있다. 최근 세 버전은 페이지에서 재생하며 이전 버전은 같은 선택기에서 GitHub 원본 다운로드로 연결한다. 24초·24fps·576프레임의 H.264/yuv420p/faststart이며 v01은 720 × 1280, v02 이후는 1080 × 1920이다.

v09는 검토 43번까지다. 보행의 분리한 눈 레이어, 짧은 깜빡임과 발 접촉 그림자를 더했다. v08은 기록 41번까지의 발 접지 보행·실제 발바닥을 맞춘 도약·착지 회복, v07은 기록 39번까지의 고개 들기와 순차 노 잡기다. v06은 기록 38번까지의 앞발 국소 관절·손 옆의 노·감정형 섬광 제거, v05는 기록 35번까지, v04는 기록 30번까지다. 기록 43번 시점의 영상별 실제 검토 횟수는 39~41회다. 적용 대상별로 계산하며 출력·자동 검사는 더하지 않는다. 아직 완성형 리깅이나 사용자가 선택한 최종 시나리오가 아니다.
