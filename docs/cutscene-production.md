...
`paddle-tail-layer-v1.png`는 꼬리만 분리한 다음 개선 후보이며 v15 MP4에는 없다. 원화 얼굴·몸체를 유지하는지 별도로 검토한다.
