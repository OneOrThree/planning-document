...
`cutscenes.html`에서 9편을 선택하고 실시간 연출과 MP4 초안을 볼 수 있다. `draft-v18`부터 `draft-v01`까지 열여덟 버전, 각각 9개 무음 MP4·포스터와 소스·에셋 지문을 가진 `manifest.json`이 있다. 총 162개는 버전별 보존본이며 서로 다른 이야기 162개가 아니다. 최근 세 버전은 페이지에서 재생하며 이전 버전은 같은 선택기에서 GitHub 원본 다운로드로 연결한다. 모두 24초 H.264/faststart이며 v01~v17은 24fps·576프레임, v18은 60fps·1,440프레임이다. v01은 720 × 1280, v02 이후는 1080 × 1920이다. 실제 스트림 검사상 v01~v12는 full-range `yuvj420p`, v13부터는 limited-range `yuv420p`다.

v18은 v17과 같은 연출을 60fps로 다시 출력한 묶음이다. 검토 59~60의 반사·준비 눈 깜빡임은 실시간에 먼저 적용되며 v18 MP4에는 없다. v17은 검토 58번까지의 작은 갈매기 활공과 노 주기에 맞춘 전진을 포함한다. v16은 검토 56번까지의 작은 꼬리 관절을 포함한다. v15는 검토 55번까지이며 천을 접어 갑판 모서리에 싣는 동작과 연속 부두 앞발을 포함한다. v14는 검토 53번까지이며 뗏목 접촉 물결·수면 미세 흐름·노를 당길 때의 작은 상체 기울기를 포함한다. v13은 검토 50번까지이며 실제 MP4에서 확인한 색 변환 문제를 수정했다. v12는 검토 49번까지의 넓은 해안·출항 카메라·노의 입수와 복귀 표현을 포함하지만, 디코딩 색 문제를 발견해 최신 공개본 승격을 보류하고 v13으로 교체했다. v11은 검토 47번까지이며 책을 보는 네 자세의 색·천 가방 레이어·나무 노의 재질과 집기를 포함한다. v10은 검토 44번까지이며 30개 파츠의 털·외곽선 색을 맞춘다. v09는 검토 43번까지의 보행 눈 깜빡임과 접촉 그림자다. v08은 기록 41번까지의 발 접지 보행·실제 발바닥을 맞춘 도약·착지 회복, v07은 기록 39번까지의 고개 들기와 순차 노 잡기다. v06은 기록 38번까지의 앞발 국소 관절·손 옆의 노·감정형 섬광 제거, v05는 기록 35번까지, v04는 기록 30번까지다. 기록 60번 시점의 영상별 실제 검토 횟수는 55~57회다. 적용 대상별로 계산하며 출력·자동 검사는 더하지 않는다. 아직 완성형 리깅이나 사용자가 선택한 최종 시나리오가 아니다.
