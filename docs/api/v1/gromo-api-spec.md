...
- 같이 집중 보너스 황금 물고기: 유효 집중 1분마다 낚는 물고기가 확률로 황금 물고기(물고기 50마리어치)가 된다. 확률은 낚는 순간 같은 섬 ACTIVE 주민 수(본인 포함·휴식 제외·최대 5명)로만 정한다 — 1명 0.1%·2명 0.2%·3명 0.4%·4명 0.6%·5명 이상 0.8%. 480마리 상한 뒤에는 추첨하지 않고, 상한 계산에서는 1마리로 세며 나머지 49마리는 별도 지급한다. 주민 누적 획득 기록·건설 각자 몫에 50마리를 합산한다. 황금 여부는 서버가 정하며 앱은 낚는 순간 1분 낚기 모션을 황금 모션으로 바꿔 보여 준다.
...
황금 물고기 여부는 1분 획득마다 서버가 (세션, 몇 번째 분) 기준으로 한 번만 정한다. 재시도·중복 요청으로 같은 1분을 다시 추첨하지 않는다. 앱은 낚는 순간 황금 모션을 보여 주므로 그 시점에 결과를 받아야 하며, 전달 방식은 P-GOLDEN-NOTIFY로 남긴다. 확률·50마리·최대 인원 5명은 60·480과 같이 서버 정책 설정으로 관리한다.
...
|P-GOLDEN-NOTIFY|황금 물고기 알림|낚는 순간 앱이 황금 여부를 받는 방식(실시간 이벤트로 보낼지, 앱이 1분마다 progress 응답으로 받을지). 앱을 꺼 둔 동안 낚은 황금 물고기를 다시 켰을 때 알려 줄지.|확률표·50마리어치·480 상한 뒤 추첨 없음. 서버가 (세션, 몇 번째 분)마다 한 번만 정하고 앱은 계산하지 않는다. 낚는 순간 1분 낚기 모션을 황금 모션으로 바꿔 보여 준다.|
