# Catus 화면별 API 스펙 · v1 협의안

버전: 0.8-proposed · 2026-09-22
...
- 화면별 스펙을 본문 원본으로 삼고, 도메인별 백엔드 전달 메모의 권한·해금·오류·읽기 모델 제안을 합쳤다. 서로 다른 endpoint 수 분류는 이 문서의 101개 기능 계약으로 통일했다.
...
- 유효 집중 60초마다 물고기 1마리를 섬 잔액에 바로 적립한다(집중 종료 때 모아 넣지 않는다). 주민 1명이 섬 1곳에서 하루 최대 480마리(유효 집중 8시간어치), 상한 뒤에도 집중 기록은 쌓인다. 이동·휴식 제외. 쉬었다 이어가도 유효 누적·할 일·획득 수·기존 자리가 유지된다.
- 같이 집중 보너스 황금 물고기: 섬마다 1분에 한 번 추첨해 황금 물고기 한 마리가 나타나면, 그 순간 ACTIVE인 주민 모두가 함께 낚고 섬 잔액에 50마리를 한 번 적립한다(인원수만큼 곱하지 않음). ACTIVE 주민이 2명 이상일 때만 추첨하며(혼자면 나타나지 않음), 확률은 추첨 순간 같은 섬 ACTIVE 주민 수(휴식 제외·최대 5명)로만 정한다 — 2명 0.4%·3명 1.2%·4명 2.4%·5명 이상 4%. 480마리 상한을 채운 주민도 ACTIVE면 인원에 포함하고, 황금 물고기는 상한과 별도로 지급한다. 함께 낚은 주민의 누적 획득 기록·건설 각자 몫에는 50÷인원(내림)씩 더하고 나머지는 섬 잔액에만 남는다. 앱은 나타난 순간 참여 주민 모두의 휴대폰에서 약 3초 컷신 영상(여러 고양이가 힘을 모아 낚아 올림)을 재생한다.
...
- 화면 잠금·다른 앱 사용·앱 종료에도 집중과 휴식은 서버 시각 기준으로 이어지고, 재실행하면 진행 중인 집중·휴식 화면으로 복귀한다. 휴식 시작부터 1시간이 지나면 서버가 집중을 자동 종료해 정상 종료와 같게 기록·퀘스트 진행에 반영하고, 다음 실행 때 결과를 한 번 보여준다. 물고기는 이미 적립돼 추가 정산이 없다. 자동 종료 전까지는 앱을 끈 휴식자도 모닥불 주민으로 표시한다.
...
- 도서관 공개 범위는 같은 섬 주민 전체 공개 고정이며, 다른 섬 방문자는 열람하지 않는다(GROMO-1904). 기록 없음(DATA_NONE)·측정 권한 없음(NO_PERMISSION)·정상 측정을 구분하고, 정상 측정은 실제0과 양수값을 모두 표현한다. 조회 권한 없음은 측정 권한 없음과 다르다.
- 전망대는 섬 간 주간 평균 랭킹만 제공. 평균은 해당 섬 집중 합계÷전체 주민 수, 일요일 00시 새 주. 초기화는 과거 기록 삭제가 아니다. 복수 소속 허용, 가입만으로 이전 소속에서 탈퇴하지 않는다.
- 친구는 전체 닉네임 정확 일치 검색과 상대 수락으로 성립한다. 보낸 요청 취소·받은 요청 거절·재신청·친구 삭제가 가능하다. 2.0은 친구 프로필과 받은 편지 상세에서 사용자 차단을 제공하고, 설정에서 내가 차단한 사용자 목록과 해제를 제공한다. 차단은 blocker 관점의 표시 필터이며 친구 관계와 편지 원문을 삭제하지 않는다. 친구 관리와 차단에 우체통 완공 조건을 붙이지 않는다.
- 모든 사용자는 메인 섬 하나를 가진다. 첫 섬 생성·가입 때 그 섬이 메인 섬이 되고 내 뗏목 내 정보에서 소속 섬 중 하나로 바꾼다. 메인 섬 탈퇴·강퇴 시 남은 섬 중 가장 최근 가입한 섬으로 바꾸고 알림을 보낸다. 메인 섬 이름은 친구 목록·친구 프로필에만 내려주고 친구가 아닌 사용자의 검색 결과에는 넣지 않는다. 메인 섬은 적립·랭킹에 영향이 없다. 앱은 마지막으로 접속한 섬에서 시작한다.
...
- 모든 주민은 마지막 소속 섬에서도 탈퇴할 수 있고, 마지막 소속 섬에서 탈퇴하면 첫 소속 선택(03 혼자 시작 / 기존 섬 참여)으로 돌아간다. 강퇴로 소속 섬이 하나도 남지 않은 사용자도 같은 곳으로 간다. 주민이 남은 섬의 방장은 위임한 뒤 탈퇴하고, 주민이 방장 한 명뿐인 섬은 삭제 범위와 다음 도착지를 확인하면 섬을 삭제하고 나간다.
...
|친구·편지|친구는 섬 소속과 무관하다. 개인 편지는 비동기 문서 단위이며 섬 전체 편지방과 수신 범위가 다르다. 사용자 차단은 blocker 관점으로 목록·검색·받은 편지를 숨긴다.|17|
...
|도서관|내/이웃 일기장. 기간 합계·그래프·할 일별 집중, 스크린타임 분류·측정 상태, 누적 물고기|주민·기록 종류·일/주/월·이전/다음 기간 선택|library-status, focus-stats, screen-stats, fish-earnings|섬 주민 전체 공개·방문자 열람 없음. 수집과 조회 해금을 분리.|
...
|67|편지 상세·작성|letter, letter-read, friends, letter-send, block-create|받은 편지의 발신자 차단 진입점; 확인 뒤 차단하고 받은 편지 정본 재조회|
...
|78|친구 관리|friend-search, friends, friend-requests, friend-send, friend-answer, friend-cancel, friend-delete, block-create|친구 프로필에서 확인 뒤 차단; 성공 후 친구 목록 정본 재조회|
...
|82|앱·알림·소리 설정|settings, settings-save, terms, blocks, block-delete|차단 목록 조회·해제; 성공 후 차단 목록 정본 재조회|
...
|rest `/screens/rest`|session·rest-members; resume·finish 제외|26 모닥불에서 쉬기|current-session·rest-members; pause 행동|조합 같음(session 경로만 바뀜). 휴식 자리 번호 대신 표시 위치|경로 이름만 갱신. 휴식 경과는 `restStartedAt`(휴식 버튼 시각) 기준|
...
집중 세션은 귀속 섬·할 일·유효 누적·휴식 구간·기존 낚시 좌표·확정 획득 수를 가진다. 종료 결과는 이미 지급된 집중 획득과 미수령 개인 퀘스트 보상·자동 지급 보너스를 구분하고, 같은 세션을 재정산하지 않는다. 물고기는 유효 집중 60초마다 섬 잔액에 바로 적립하며, 종료 결과의 집중 물고기는 이번 집중에서 이미 적립한 수다.
...
황금 물고기는 섬 단위로 서버가 1분마다 (섬, 추첨 시각) 기준으로 한 번만 추첨한다. 재시도·중복 처리로 같은 추첨을 다시 하지 않는다. 나타나면 섬 잔액 50마리 적립과 참여 주민별 기록·각자 몫 배분을 같은 처리로 확정한다. 참여 주민 모두의 휴대폰에서 같은 순간 컷신을 재생해야 하므로 참여 주민 목록과 함께 섬 실시간 채널로 알리는 것을 전제로 하며, 세부 전달 방식은 P-GOLDEN-NOTIFY로 남긴다. 확률·50마리·최대 인원 5명은 60·480과 같이 서버 정책 설정으로 관리한다.

재실행/재접속은 내 현재 집중과 현재 섬 주민 snapshot을 조회한다. 복구된 획득은 같은 구간을 중복 지급하지 않는다. 종료 응답 유실은 같은 정산 결과를 재조회/재시도해 복구한다. 백그라운드·앱 종료에도 집중·휴식은 이어진다. 휴식 시작부터 1시간이 지나면 서버가 자동 종료하고, 앱은 다음 실행 때 그 결과를 한 번 보여준다. 집중 상태로 방치할 때의 최대 시간은 P-FOCUS-IDLE로 남긴다.
...
- 입력·조회: 앞뒤 공백을 제거한 닉네임·catColor·mainIslandId(내가 속한 섬 중 하나); 닉네임은 대소문자를 구분하지 않고 고유
...
- 입력·조회: 대상 주민·확인
- 반환·변경: 소속 해제·건설 대상 제외·공동 잔액 유지; 대상의 메인 섬이면 가장 최근 가입한 남은 섬으로 변경·알림; 소속 섬이 하나도 남지 않으면 대상은 첫 섬 선택 화면으로 이동
- 권한: 방장
- 해금·조건: 회관
...
- 반환·변경: 일반 탈퇴는 소속 해제·남은 메인 섬 또는 첫 섬 선택 화면, 방장 1인 섬은 공동 데이터 삭제·남은 메인 섬 또는 첫 섬 선택 화면; 탈퇴한 섬이 메인 섬이면 가장 최근 가입한 남은 섬으로 변경·알림
- 권한: 본인; 주민이 남은 방장은 위임 후 실행
- 해금·조건: 마지막 소속 섬도 탈퇴 가능(첫 섬 선택 화면으로 이동). 주민이 방장 한 명뿐인 섬은 삭제 범위와 도착지를 확인한 뒤 섬을 삭제하고 탈퇴
...
친구는 섬 소속과 무관하다. 개인 편지는 비동기 문서 단위이며 섬 전체 편지방과 수신 범위가 다르다. 사용자 차단은 친구 관계·편지 원문을 삭제하지 않고, 차단한 사람의 화면에서 상대를 숨긴다.
...
- 호출 화면: 설정 → 차단 목록
- 입력·조회: 인증 세션의 blocker
- 반환·변경: `[{userId, nickname}]`; 내가 차단한 사용자만 반환하며 반대 방향 차단은 노출하지 않음
- 권한: 본인
- 해금·조건: 우체통 건설 조건 없음
- 구현 증거: GROMO-1975 · phone PR #929

#### block-create · 사용자 차단

`POST /blocks`

- 호출 화면: 친구 프로필, 받은 편지 상세
- 입력·조회: JSON `{ "blockedUserId": "uuid" }`; 확인 단계 뒤 호출
- 반환·변경: `204 No Content`; 같은 방향 재호출도 멱등 성공
- 후속 조회: 친구 프로필에서는 `friends`와 현재 검색 결과, 편지 상세에서는 `letters-received`를 다시 조회한다. 성공 응답 전에 로컬에서만 숨김 처리하지 않는다.
- 서버 표시 필터: 차단한 상대를 친구 목록·친구 검색·받은 편지에서 제외한다. 보낸 편지와 친구 관계·편지 원문은 삭제하지 않는다.
- 오류: 자기 차단은 `400 SELF_BLOCK`, 탈퇴·없는 대상은 대상 없음 오류
- 권한: 인증 본인
- 해금·조건: 우체통 건설 조건 없음
- 구현 증거: GROMO-1975 · phone PR #929

#### block-delete · 사용자 차단 해제

`DELETE /blocks/{blockedUserId}`

- 호출 화면: 설정 → 차단 목록
- 입력·조회: 해제할 상대 userId
- 반환·변경: `204 No Content`; 이미 해제됐거나 대상 사용자 행이 없어도 멱등 성공
- 후속 조회: `blocks`를 다시 조회해 서버 정본 목록을 표시한다.
- 권한: 본인
- 해금·조건: 우체통 건설 조건 없음
- 구현 증거: GROMO-1975 · phone PR #929
...
- 반환·변경: 사용자 식별·닉네임·고양이·친구/요청 상태; 친구가 아니면 메인 섬 이름 없음
- 권한: 인증 회원
- 해금·조건: 우체통 건설 조건 없음
...
|P-FOCUS-IDLE|집중 방치·강퇴 적립분|앱을 꺼 둔 채 집중 상태로 방치할 때 자동 종료할 최대 시간. 강퇴된 주민이 진행 중이던 집중에서 이미 적립한 물고기의 처리.|화면 잠금·다른 앱·앱 종료에도 집중 유지. 60초마다 섬 잔액에 즉시 적립, 하루 480마리 상한. 휴식은 1시간 방치 시 자동 종료.|
...
|P-RECORD-SCOPE|도서관 조회 범위|완공 뒤 건설 이전 기록 조회 여부, 내 일기장의 현재 섬/전체 개인 범위, 로그인 없는 외부 열람.|미완공 동안 이전 기록 조회 제한, 이번 결과는 허용, 타 섬 방문자 열람 없음·섬 주민 전체 공개 고정.|
|P-POSITION|낚시 위치|다른 주민과 위치가 겹칠 때의 처리.|자유 지도 좌표·자리 번호 없음.|
|P-GOLDEN-NOTIFY|황금 물고기 알림|섬 실시간 이벤트의 이름·내용(참여 주민·배분량), 재연결 때 놓친 이벤트 복구, 앱을 꺼 둔 채 집중 중인 주민에게 다시 켰을 때 알려 줄지.|섬마다 1분 1회 서버 추첨, 확률표, 섬 잔액 50마리 1회, 참여 주민 50÷인원(내림) 배분, 480 상한과 별도. ACTIVE 2명 이상일 때만 추첨. 앱은 계산하지 않고 알림을 받아 참여 주민 모두의 휴대폰에서 약 3초 컷신을 재생한다.|
|P-FRIEND|친구 검색·프로필 표시|복수 소속 친구의 메인 섬 외 추가 정보를 어디까지 표시할지.|대소문자 없이 정확한 닉네임 검색, 수락으로 친구, 요청 취소·거절·삭제·재신청, 삭제 시 미열람 편지 삭제. 차단은 2.0 포함이며 blocker 관점 표시 필터로 동작.|
...
|P-MODERATION|공지·댓글·신고|본인 댓글 삭제·방장의 다른 주민 댓글 삭제 권한, 공지 작성자/운영자 예외, 신고 제공 범위.|방장 공지 작성·수정, 주민 댓글. 사용자 차단은 2.0 포함이며 친구 프로필·받은 편지 상세에서 제공.|
