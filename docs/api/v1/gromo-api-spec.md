...
|우체통|우리 섬 편지방 + 친구 받은/보낸 편지. 읽지 않은 수·편지 상세·작성|섬 글 남기기, 친구 선택·편지 발송·확인(닫으면 삭제)|island-messages, island-message, letters-received, letters-sent, letter, letter-send, letter-read|친구 검색·관리는 내 뗏목. 개인 편지는 비동기 발송, 확인되면 양쪽에서 삭제.|
...
|67|편지 상세·작성|letter, letter-read, friends, letter-send|개인 편지 상세·닫으면 삭제·친구 선택·발송|
...
트리거 행동 제외(B12) 확인: 구매·건설·목표 선택·가입·전송·종료·휴식 전환·보상받기는 지금도 별도 명령 계약이라 기준과 맞다. 다만 `progress`(PUT 진행 검증, 22·23)·`letter-read`(PATCH 확인·삭제, 67)·`invite-resolve`(POST 코드 확인, 04·59)는 화면에 머무는 중 부를 수 있어 집계 GET에 넣지 않는다고 명시가 필요하고, `playback`은 1784에선 재생 상태 조회·지금은 재생 변경(PUT)이라 이름을 구분해야 한다.
...
- 입력·조회: 닉네임·catColor·메인 섬(소속 섬 중 하나)
...
- 입력·조회: 대상 주민·확인
- 반환·변경: 소속 해제·건설 대상 제외·공동 잔액 유지; 대상의 메인 섬이면 가장 최근 가입한 남은 섬으로 변경·알림; 소속된 마지막 섬 강퇴 처리 미정
- 권한: 방장
- 해금·조건: 회관
...
#### letter-read · 받은 편지 확인

`PATCH /v1/me/letters/{letterId}/read`

- 입력·조회: 접근 islandId·편지 ID
- 반환·변경: 편지를 닫을 때 확인 처리하고 받은·보낸 목록에서 모두 삭제; 이미 삭제된 편지도 같은 결과
- 권한: 수신자 본인
- 해금·조건: 우체통 진입 조건 검사
...
|letter.created / letter.read|편지 발신자·수신자만|개인 편지 참조·확인 후 삭제·읽지 않은 수|실시간 알림이 가능해도 채팅방 모델은 아님|
...
|P-LAST-ISLAND|마지막 섬 강퇴|소속된 마지막 섬에서 강퇴된 사용자의 처리.|메인 섬 하나 보유, 메인 섬 탈퇴·강퇴 시 가장 최근 가입한 남은 섬으로 변경·알림, 소속된 마지막 섬 탈퇴 불가, 앱은 마지막 접속 섬에서 시작.|
