...
- 친구는 상대 수락으로 성립. 보낸 요청 취소·받은 요청 거절·재신청·친구 삭제 가능. 친구 관리에 우체통 완공 조건을 붙이지 않는다.
- 우체통은 섬 전체 편지방과 친구 비동기 편지. 개인 편지는 받은·보낸·상세·읽음 상태이며 실시간 1:1 채팅방·입력중 상태·대화방 API는 없다.
...
|우체통|우리 섬 편지방 + 친구 받은/보낸 편지. 읽지 않은 수·편지 상세·작성|섬 글 남기기, 친구 선택·편지 발송·읽음 처리|island-messages, island-message, letters-received, letters-sent, letter, letter-send, letter-read|친구 검색·관리는 내 뗏목. 개인 편지는 비동기 발송.|
...
|우체통|섬 전체 편지, 친구 요청 상태, 받은·보낸 편지, 편지 상세·읽음 상태|
...
|02|내 고양이와 이름|me, profile|고양이 털색 6종; 닉네임 상세 정책 미정|
...
|67|편지 상세·작성|letter, letter-read, friends, letter-send|개인 편지 상세·친구 선택·발송·읽음|
...
|79|친구 찾기|friend-search, friends, friend-requests, friend-send, friend-answer, friend-cancel, friend-delete|친구 검색·요청; 검색 식별자 미정|
...
- 입력·조회: 본인·확인
- 반환·변경: 소속 해제·접근 가능한 다음 섬/첫 선택; 공동 물고기 유지
- 권한: 본인; 방장은 위임/섬 종료 조건
- 해금·조건: 위임·마지막 주민 처리 검사
...
- 입력·조회: 검색어·cursor; 식별자 정책 미정
- 반환·변경: 사용자 식별·닉네임·고양이·친구/요청 상태
- 권한: 인증 사용자; 검색 노출 정책 미정
- 해금·조건: 우체통 건설 조건 없음
...
- 반환·변경: 편지 목록·발신자·시각·요약·읽음·읽지 않은 수
- 권한: 수신자 본인; 게스트 읽기 범위 미정
- 해금·조건: 접근 섬 우체통 완공; 상대 조건 미정
...
#### letter-read · 받은 편지 읽음

`PATCH /v1/me/letters/{letterId}/read`

- 입력·조회: 접근 islandId·편지 ID
- 반환·변경: 읽음 상태·시각; 여러 번 읽어도 같은 결과
- 권한: 수신자 본인
- 해금·조건: 우체통 진입 조건 검사
...
|letter.created / letter.read|편지 발신자·수신자만|개인 편지 참조·읽음·읽지 않은 수|실시간 알림이 가능해도 채팅방 모델은 아님|
...
|P-FRIEND|친구 검색·삭제|닉네임·고유 코드 등 검색 식별자와 노출 기준, 복수 소속의 표시 섬, 친구 삭제 후 기존 편지 보존·재발송.|수락으로 친구, 요청 취소·거절·삭제·재신청, 섬 소속과 별개.|
|P-MAIL-GATE|편지·게스트 접근|받는 친구 쪽에도 우체통 완공이 필요한지. 게스트의 편지 읽기·친구 요청 수락을 허용할지.|우체통 완공 조건 유지, 친구 추가·편지 발송은 회원 전환, 실시간 1:1 채팅방 없음.|
