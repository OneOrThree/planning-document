...
- 휴식은 원래 섬 모닥불에서 독서. 이어가기는 같은 낚시 자리 도착 후 재개. 휴식 종료는 같은 집중 종료 → 결과 → 홈. 낚시섬 집중 종료는 결과 → 귀환 항해 → 홈.
...
- 후속 건설 대상은 목표 선택 당시 주민으로 고정. 새 가입자 자동 추가 없음, 탈퇴/강퇴자는 제외. 공사 전 목표 변경 시 현재 주민으로 다시 설정한다. 이미 모은 각자 몫을 이어서 셀지는 미정이다.
- 후속 건설은 기존 보유분이나 이전 건물에서 모은 각자 몫을 요구량에 인정하지 않는다(도서관 누적 획득 기록은 유지). 전원이 각자 몫(총액÷대상 인원, 올림)을 채우고 섬 잔액이 총액 이상이면 도장, 방장 공사 시작 시만 차감. 잔액이 줄면 도장은 해제·복구되고 각자 몫 진행은 유지된다. 시작한 공사는 자기 차감 때문에 되돌리지 않는다.
...
- 모든 사용자는 메인 섬 하나를 가진다. 첫 섬 생성·가입 때 그 섬이 메인 섬이 되고 내 뗏목 내 정보에서 소속 섬 중 하나로 바꾼다. 메인 섬 탈퇴·강퇴 시 남은 섬 중 가장 최근 가입한 섬으로 바꾸고 알림을 보낸다. 메인 섬 이름은 친구 목록·친구 프로필에만 내려주고 친구가 아닌 사용자의 검색 결과에는 넣지 않는다. 메인 섬은 적립·랭킹에 영향이 없다. 소속된 마지막 섬은 탈퇴할 수 없다. 앱은 마지막으로 접속한 섬에서 시작한다.
- 친구는 상대 수락으로 성립. 보낸 요청 취소·받은 요청 거절·거절 뒤 즉시 재신청·친구 삭제 가능. 친구 관리에 우체통 완공 조건을 붙이지 않는다. 닉네임은 대소문자 구분 없이 중복 불가, 생성·변경 때 앞뒤 공백을 제거해 저장한다. 친구 검색은 대소문자 구분 없이 닉네임이 정확히 일치할 때만 반환하고 유사 닉네임·본인·탈퇴 사용자는 제외한다. 친구 삭제 시 서로 편지 발송 불가, 확인하지 않은 편지도 삭제. 차단은 2.0.0에서 제공하지 않는다.
- 우체통은 섬 전체 편지방과 친구 비동기 편지. 친구 편지는 기록으로 남기지 않는다. 받는 사람이 열었다가 닫으면 삭제하고, 보낸 편지 목록에는 상대가 확인하기 전까지만 남기며 확인되면 양쪽에서 삭제한다. 받는 친구 섬의 우체통 완공과 무관하게 발송하고, 받은 편지는 받는 사람이 속한 섬 중 우체통 완공 섬에서 연다. 실시간 1:1 채팅방·입력중 상태·대화방 API는 없다.
...
- 목록은 cursor 페이지네이션. 검색어·기간·주민·분류 변경 시 cursor 초기화. 읽음·보상·확인 상태는 작성자/수신자/회차 소유 범위로 검사한다.
...
- 방장 강퇴·탈퇴·위임·섬 종료 뒤 접근·구독을 다시 검사한다. 공동 자산은 남고 개인 의상은 유지한다. 마지막 주민/진행 집중 정리의 상세 예외는 미정 표시를 따른다.
...
|친구·편지|친구는 섬 소속과 무관하다. 개인 편지는 비동기 문서 단위이며 섬 전체 편지방과 수신 범위가 다르다. 받는 사람이 확인하면 삭제되어 기록으로 남지 않는다.|14|
...
|마을회관|책상에서 섬 관리·목각 건물·공동 가계부. 정보/주민/가입 신청, 월별 잔액·적립·지출|방장 정보 수정·가입 승인·강퇴·위임·목표 선택, 주민 열람·본인 탈퇴|island, members, join-incoming, ledger, buildables, build-target|통계는 도서관. 주민 목록을 방장 전용으로 막지 않는다.|
...
|우체통|우리 섬 편지방 + 친구 받은/보낸 편지. 읽지 않은 수·편지 상세·작성|섬 글 남기기, 친구 선택·편지 발송·확인(닫으면 삭제)|island-messages, island-message, letters-received, letters-sent, letter, letter-send, letter-close|친구 검색·관리는 내 뗏목. 개인 편지는 비동기 발송, 확인되면 양쪽에서 삭제.|
...
|우체통|섬 전체 편지, 친구 요청 상태, 확인 전 받은·보낸 편지, 편지 상세|
...
|02|내 고양이와 이름|me, profile|고양이 털색 6종; 닉네임 대소문자 무시 중복 불가·앞뒤 공백 제거, 그 밖의 상세 정책 미정|
...
|26|모닥불에서 쉬기|pause, rest-members, current-session|유효 집중 중지·휴식 시간 표시·기존 자리 보존|
|27|휴식 종료 · 이번 집중 결과|finish, session|같은 집중 1회 정산 → 결과 → 홈; 귀환 항해 없음|
...
|66|친구 편지함|letters-received, letters-sent, friends|확인 전 받은/보낸 편지·읽지 않은 수|
|67|편지 상세·작성|letter, letter-close, friends, letter-send|개인 편지 상세·닫으면 삭제·친구 선택·발송|
...
|79|친구 찾기|friend-search, friends, friend-requests, friend-send, friend-answer, friend-cancel, friend-delete|친구 검색·요청; 닉네임 정확 일치|
...
트리거 행동 제외(B12) 확인: 구매·건설·목표 선택·가입·전송·종료·휴식 전환·보상받기는 지금도 별도 명령 계약이라 기준과 맞다. 다만 `progress`(PUT 진행 검증, 22·23)·`letter-close`(DELETE 닫기·삭제, 67)·`invite-resolve`(POST 코드 확인, 04·59)는 화면에 머무는 중 부를 수 있어 집계 GET에 넣지 않는다고 명시가 필요하고, `playback`은 1784에선 재생 상태 조회·지금은 재생 변경(PUT)이라 이름을 구분해야 한다.

## 집중·휴식·이동 계약

섬 홈 → 부두까지 걷기 → 항해 → 낚시섬 도착 → 자유 위치까지 걷기 → 할 일 준비 → 집중 시작. 앞의 모든 이동·준비는 집중 시간에서 제외한다. 배를 눌러 준비 전 귀환할 때는 집중 정산이 없다.

집중 → 휴식 요청으로 유효 집중 중지 → 뗏목까지 걷기·귀환 항해 → 원래 섬 모닥불 휴식. 휴식 경과 표시의 시작을 요청 시점/모닥불 도착 중 언제로 할지는 화면과 함께 확인한다. 어떤 경우든 이동이 유효 집중에 포함되지는 않는다.
...
실시간 장면은 scene·고양이 외양·지도 기준 위치/목적지·시간 기준을 공유한다. 경로 찾기·좌우 반전·걷기/깜빡임/독서/낚시 프레임은 앱이 그린다. 서버는 정상 활동 위치를 검증할 수 있으나 임의 자리 번호나 물고기 그림 단계별 API는 필요 없다.
...
후속 건설(축음기~상점)의 총액은 대상 인원과 무관한 고정값이다. 대상 주민은 각자 "총액 ÷ 대상 인원(올림)"을 각자 몫으로 목표 선택 후부터 모은다. 전원이 각자 몫을 채우고 섬 잔액이 총액 이상이면 완료 도장, 방장이 건설하기를 누르면 총액을 차감하고 공사를 시작한다. 기존 보유분이나 이전 건물에서 모은 각자 몫은 다음 건물 요구량에 인정하지 않는다. 다만 도서관의 주민별 누적 획득 기록은 그대로 유지하며 초기화하지 않는다. 다음 건물 목표는 공사 완료 후에만 열리며, 공사 중 집중 획득은 다음 건물의 각자 몫에 들어가지 않는다. 첫 회관/게시판은 공동 초기 목표로 섬 잔액 합산만으로 판정하며 후속 각자 몫 방식으로 계산하지 않는다. 게시판의 추가240 준비량 산정 범위는 서버 계약 시 문장으로 명확히 한다.

건설 상태는 목표 준비 → 달성+잔액 충족(도장) → 방장 실행·차감 → 공사 → 완공. 목표 변경/대상 제외·잔액 사용이 있으면 서버가 다시 계산한다. 도서관 전에도 건설에 필요한 주민별 달성 정보는 제공하되 전체 누적 기록·기간 통계는 도서관에서 조회한다.
...
- 입력·조회: 없음
- 반환·변경: 사용자 ID·닉네임·고양이 털색·착용품·현재 소속 섬(마지막 접속 섬)·메인 섬·온보딩 상태
- 권한: 본인
- 해금·조건: 처음부터
...
- 입력·조회: 닉네임·catColor·mainIslandId(내가 속한 섬 중 하나)
- 반환·변경: 저장한 프로필; 닉네임은 앞뒤 공백 제거 후 대소문자 무시 중복 검사; 단일 로그인 수단은 변경하지 않음
- 권한: 본인
- 해금·조건: 처음부터
...
- 입력·조회: 대상 주민·확인
- 반환·변경: 소속 해제·건설 대상 제외·공동 잔액 유지; 대상의 메인 섬이면 가장 최근 가입한 남은 섬으로 변경·알림; 소속된 마지막 섬 강퇴 처리는 미정(P-LAST-KICK)
- 권한: 방장
- 해금·조건: 회관
...
- 입력·조회: 본인·확인
- 반환·변경: 소속 해제·접근 가능한 다음 섬; 메인 섬이면 가장 최근 가입한 남은 섬으로 변경·알림; 공동 물고기 유지
- 권한: 본인; 방장은 위임/섬 종료 조건
- 해금·조건: 위임·마지막 주민 처리 검사; 소속된 마지막 섬은 탈퇴 거부
...
- 입력·조회: 목적지 소속 섬
- 반환·변경: 새 현재 소속 섬·이동 문맥; 마지막 접속 섬으로 저장해 앱 시작에 사용
- 권한: 해당 섬 주민 본인
- 해금·조건: 전망대·진행 집중 종료
...
- 입력·조회: 본인 세션·전환 문맥
- 반환·변경: 유효 집중 중지·기존 자리/획득 유지·이동/휴식 시간 기준
- 권한: 세션 소유자 본인
- 해금·조건: 집중 중; 이동 시간은 집중 제외
...
- 입력·조회: 섬 ID
- 반환·변경: 고양이·이름·휴식 시작/경과 기준·표시 위치
- 권한: 같은 섬 주민
- 해금·조건: 기본 모닥불; 다른 시설 해금 불필요
...
친구는 섬 소속과 무관하다. 개인 편지는 비동기 문서 단위이며 섬 전체 편지방과 수신 범위가 다르다. 받는 사람이 확인하면 삭제되어 기록으로 남지 않는다.

#### friend-search · 친구 찾기

`GET /v1/users/search`

- 입력·조회: 닉네임·cursor; 대소문자 무시 정확 일치만
- 반환·변경: 사용자 식별·닉네임·고양이·친구/요청 상태; 유사 닉네임·본인·탈퇴 사용자 제외, 친구가 아니면 메인 섬 이름 없음
- 권한: 인증 사용자
- 해금·조건: 우체통 건설 조건 없음
...
- 입력·조회: cursor
- 반환·변경: 친구 프로필·친구 관계·친구의 메인 섬 이름
- 권한: 본인
- 해금·조건: 우체통 건설 조건 없음
...
- 입력·조회: accept|reject
- 반환·변경: 수락 시 양방향 친구; 거절 후 바로 재신청 가능
...
- 입력·조회: 친구 ID
- 반환·변경: 관계 해제·서로 편지 발송 불가·확인하지 않은 편지 삭제
- 권한: 본인
- 해금·조건: 우체통 건설 조건 없음
...
- 반환·변경: 확인하지 않은 편지 목록·발신자·시각·요약·읽지 않은 수
- 권한: 수신자 본인; 게스트 읽기 범위 미정
- 해금·조건: 접근 섬 우체통 완공(내가 속한 섬 중 완공된 섬이면 열람)
...
- 입력·조회: 접근 islandId·cursor
- 반환·변경: 상대가 확인하기 전 편지 목록·수신자·시각·요약; 확인된 편지는 제외
- 권한: 발신자 본인
- 해금·조건: 접근 섬 우체통 완공
...
- 반환·변경: 발신자·수신자·본문·시각; 확인 전 편지만 조회
- 권한: 해당 편지 발신자/수신자 본인
- 해금·조건: 우체통 진입 조건 검사
...
- 해금·조건: 접근 섬 우체통 완공; 받는 친구 섬의 우체통 완공은 요구하지 않음

#### letter-close · 받은 편지 닫기·삭제

`DELETE /v1/me/letters/{letterId}`

- 입력·조회: 접근 islandId·편지 ID
- 반환·변경: 받은·보낸 목록에서 모두 삭제; 읽음 상태는 저장하지 않음; 이미 삭제된 편지도 같은 결과
- 권한: 수신자 본인
- 해금·조건: 우체통 진입 조건 검사
...
|letter.created / letter.deleted|편지 발신자·수신자만|개인 편지 참조·닫아서 삭제·읽지 않은 수|실시간 알림이 가능해도 채팅방 모델은 아님|
|main-island.updated|본인만|바뀐 메인 섬|탈퇴·강퇴로 자동 변경될 때 알림|
...
|P-EARNED-SCOPE|각자 몫 포함 범위|공사 전 목표를 바꿀 때 이미 모은 각자 몫을 이어서 셀지. 일일 퀘스트 보상을 각자 몫에 넣을지.|공동 잔액에는 적립. 구매·건설로 주민 누적은 감소하지 않는다.|
|P-SESSION-TIME|집중·휴식 시간|휴식 경과 시간을 버튼 누른 시점부터 셀지, 모닥불 도착부터 셀지. 화면 잠금·백그라운드·앱 종료 때 집중을 유지/중지/종료하는 기준.|이동·휴식은 유효 집중에서 제외. 휴식 후 같은 세션·기존 자리로 재개.|
...
|P-LAST-KICK|마지막 섬 강퇴|소속된 마지막 섬에서 강퇴된 사용자의 처리.|메인 섬 하나 보유, 메인 섬 탈퇴·강퇴 시 가장 최근 가입한 남은 섬으로 변경·알림, 소속된 마지막 섬 탈퇴 불가, 앱은 마지막 접속 섬에서 시작.|
|P-MAIL-GATE|편지·게스트 접근|게스트의 편지 읽기·친구 요청 수락을 허용할지.|우체통 완공 조건 유지, 받는 친구 섬의 우체통 완공은 요구하지 않음, 친구 추가·편지 발송은 회원 전환, 실시간 1:1 채팅방 없음.|
...
|P-MODERATION|공지·댓글·신고|본인 댓글 삭제·방장의 다른 주민 댓글 삭제 권한, 공지 작성자/운영자 예외, 신고 제공 범위.|방장 공지 작성·수정, 주민 댓글. 차단은 2.0.0에서 제공하지 않음.|
