...
(() => {
  ...
  function load(){
    ...
    loaded=fetch(base+'walk-v1.json').then(r=>{if(!r.ok)throw Error('보행 자세 명세 로드 실패');return r.json();}).then(async spec=>{
      data=spec;await Promise.all([spec.neutral,...spec.frames,...(spec.blink||[]),...(spec.jump||[]),...(spec.reach||[]),...(spec.reachBlink||[]),...(spec.recovery||[])].map(f=>new Promise((resolve,reject)=>{const img=new Image();img.onload=()=>{images.set(f.file,img);resolve();};img.onerror=()=>reject(Error('개별 자세 로드 실패: '+f.file));img.src=base+f.file;})));return spec;
    });return loaded;
  }
  function draw(c,x,foot,size,time,options={}){
    if(!data||images.size!==data.frames.length+1+(data.blink?.length||0)+(data.jump?.length||0)+(data.reach?.length||0)+(data.reachBlink?.length||0)+(data.recovery?.length||0))throw Error('개별 자세는 load() 완료 후 그려야 합니다.');
    ...
    return{state:recovering?'recovery-individual-poses':reaching?'reach-individual-poses':jumping?'jump-individual-poses':index<0?(blinkIndex<0?'neutral':'blink-individual-poses'):'walk-individual-poses',frame:recovering?recoveryIndex:reaching?reachIndex:jumping?jumpIndex:index,blinkFrame:blinkIndex,phase,baseline:frame.originY,hand,status:data.status};
  }
