...
(() => {
  ...
  const entries = [
    {"id":"policy-golden-fish","status":"confirmed","title":"같이 집중할수록 황금 물고기가 자주 나타나고, 다 같이 낚는다","body":"같이 집중해도 혼자일 때와 보상이 같던 문제를 황금 물고기로 푼다. 섬마다 1분에 한 번 추첨해 황금 물고기가 나타나면 그 순간 집중 중인 주민 모두가 함께 낚는 모션을 한 뒤 섬 잔액에 50마리를 한 번 더한다. 확률은 추첨 순간 같은 섬에서 집중 중인 주민 수(휴식 제외·최대 5명)로만 정하며 1분당 혼자 0.1%, 2명 0.4%, 3명 1.2%, 4명 2.4%, 5명 이상 4%다. 480마리 상한과 별도로 지급하고 상한을 채운 주민도 인원에 넣는다. 50마리는 함께 낚은 주민의 누적 기록·건설 각자 몫에 인원수로 나눠(내림) 더한다. 황금 여부는 서버가 정한다. 혼자 집중할 때의 기본 보상은 1분당 1마리 그대로다.","link":"feature-inventory.html?mode=detail#feature-D22","date":"2026.09.19","source":"GROMO-1921 · 사용자 결정"},
