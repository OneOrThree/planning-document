...
(() => {
  ...
  const entries = [
    ...
    {"id":"policy-fish-currency","status":"confirmed","title":"재화는 섬 소유 물고기 하나로 합친다","body":"마을 포인트·개인 물고기 지갑·게시판 완성 전후 귀속 전환을 없앤다. 유효 집중 5분(300초)마다 물고기 1마리를 낚고, 집중한 섬에만 적립한다. 휴식 시간은 유효 집중에서 제외한다. 탈퇴해도 모은 물고기는 섬에 남는다. 물고기 잔액과 주민별 누적 획득 기록은 구분하며, 구매·건설로 잔액이 줄어도 누적 기록은 줄지 않는다. 기획 반영이며 실제 앱 구현 완료와 구분한다.","link":"feature-inventory.html?mode=detail#feature-L03","date":"2026.09.14","source":"사용자 결정 · 현재 정책 2026-09-14"},
    {"id":"policy-building-quest","status":"confirmed","title":"회관·게시판 다음 건물은 건설 퀘스트로 짓는다","body":"마을회관(초기 공동 누적 20마리)과 게시판(추가 40마리)은 기존 순서를 유지한다. 이후 방장이 회관에서 다음 건물을 고르면 게시판에 건물 건설 퀘스트가 생긴다. 도서관·전망대·우체통·축음기는 순서가 자유이고 상점은 다른 모든 건물 완공 후에 고른다. 한 번에 한 건물만 진행한다. 대상은 목표를 고른 시점의 주민으로 고정하고 탈퇴하거나 강퇴된 주민은 뺀다. 대상 전원이 1인당 누적 요구량을 채우고 섬 잔액도 충분하면 완료 도장을 표시한다. 방장이 건설하기를 누를 때 대상 인원 × 1인당 요구량을 차감하고 공사를 시작한다. 1인당 요구량(축음기 10·도서관 20·우체통 30·전망대 40·상점 50마리)과 공사 시간(회관 1분~상점 1시간)은 위임받아 채택한 초기값이며 운영 후 조정할 수 있다.","link":"feature-inventory.html?tab=growth","date":"2026.09.14","source":"사용자 결정 · 현재 정책 2026-09-14"},
    {"id":"policy-daily-quest-reward","status":"confirmed","title":"일일 퀘스트 보상은 섬 물고기로 준다","body":"집중·스크린타임 퀘스트는 모두 일일 퀘스트이며 다음 날 새 회차가 된다. 주민 한 명이 달성하면 섬에 물고기 2마리, 전원이 달성하면 대상 주민 수 × 1마리를 더 준다. 같은 회차에서 각 주민의 달성 보상과 전원 보너스는 한 번씩만 지급한다. 퀘스트 성공 1인 10P 안은 쓰지 않는다. 건설 퀘스트는 추가 물고기 없이 건물 해금이 보상이다.","link":"feature-inventory.html?mode=detail#feature-L05","date":"2026.09.14","source":"사용자 결정 · 현재 정책 2026-09-14"},
