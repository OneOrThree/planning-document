...
(() => {
  ...
  const entries = [
    ...
    {"id":"policy-join-capacity","status":"confirmed","title":"가입 방식과 정원은 방장이 정하고 신청은 취소할 수 있다","body":"섬을 만들 때 방장이 가입 방식(승인 필요·승인 불필요)과 정원을 정하고, 정원은 마을회관에서 수정한다. 승인 대기 중인 가입 신청은 취소할 수 있고 다시 신청할 수 있다. 복수 소속을 허용하며 새로 가입해도 기존 섬에서 탈퇴하지 않는다. 2026.09.16 추가 결정(GROMO-1867): 모든 사용자는 메인 섬을 하나 가진다. 첫 섬을 만들거나 가입하면 그 섬이 메인 섬이 되고, 내 뗏목의 내 정보에서 내가 속한 섬 중 하나로 바꿀 수 있다. 메인 섬에서 탈퇴하거나 강퇴되면 남은 섬 중 가장 최근에 가입한 섬이 메인 섬이 되고 알림을 보낸다. 메인 섬 이름은 친구 목록과 친구 프로필에만 표시하고 친구가 아닌 사용자의 검색 결과에는 표시하지 않으며, 물고기·집중 기록 적립과 랭킹에 영향을 주지 않는다. 소속된 마지막 섬에서는 탈퇴할 수 없어 다른 섬에 가입하거나 새 섬을 만든 뒤 탈퇴한다. 앱을 켜면 마지막으로 접속한 섬에서 시작한다. 소속된 마지막 섬에서 강퇴된 사용자의 처리는 추가 결정이 필요하다.","link":"feature-inventory.html?mode=detail#feature-B10","date":"2026.09.14","source":"사용자 결정 · 현재 정책 2026-09-14"},
    ...
    {"id":"policy-art-guides","status":"confirmed","title":"앵무새 안내 NPC와 건물 외형 기준","body":"빨강·노랑·파랑 앵무새는 섬에 원래 살던 안내 NPC다. 첫 도착·낚시 집중·모닥불 휴식·건물과 공동 기능을 한 번에 하나씩 소개하고 건너뛰기·다시 보기를 허용하며 주민·퀘스트 인원으로 세지 않는다. 공동 음악 장치의 외형·명칭은 꽃나팔 방송기이며 이전 축음기와 같은 장치다. 도서관은 세이지 녹색 지붕의 팔각 2층 책탑이고, 작은 낚시 섬 부두는 고양이 약 10마리가 앉는 장면을 목표로 기본 뗏목 한 대만 둔다. 장면 수용 목표는 섬 정원과 별개다.","link":"feature-inventory.html?mode=detail#feature-N09","date":"2026.09.14","source":"사용자 결정 · 현재 정책 2026-09-14"},
    ...
    {"id":"letters","status":"confirmed","title":"콕찌르기 없이, 섬 전체 편지방","source":"이전 사용자 결정 + 04 수정본","body":"우체통에서 같은 섬 전체에 자유롭게 글을 남긴다. 개인 수신자 선택·읽음 표시·콕찌르기는 없다. 외부 친구의 첫 초대는 별도 진입 흐름이다. 2026-09-14 현재 정책에서 섬 전체 편지방의 이 원칙은 유지하고, 친구에게 보내는 비동기 편지를 우체통에 따로 추가했다.","link":"feature-inventory.html?mode=detail#feature-I06"},
