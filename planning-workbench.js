...
(() => {
  ...
  const entries = [
    ...
    {"id":"policy-building-quest","status":"confirmed","title":"회관·게시판 다음 건물은 건설 퀘스트로 짓는다","body":"마을회관(초기 공동 누적 60마리)과 게시판(추가 240마리)은 기존 순서를 유지한다. 이후 방장이 회관에서 다음 건물을 고르면 게시판에 건물 건설 퀘스트가 생긴다. 도서관·전망대·우체통·축음기는 순서가 자유이고 상점은 다른 모든 건물 완공 후에 고른다. 한 번에 한 건물만 진행한다. 대상은 목표를 고른 시점의 주민으로 고정하고 탈퇴하거나 강퇴된 주민은 뺀다. 게시판 다음 건물은 섬 인원과 상관없는 고정 총액을 대상 인원으로 나눈 각자 몫(올림)을 대상 주민이 목표를 고른 뒤부터 모은다. 대상 전원이 각자 몫을 채우고 섬 잔액도 총액 이상이면 완료 도장을 표시한다. 방장이 건설하기를 누를 때 총액을 차감하고 공사를 시작한다. 총액(축음기 1,360·도서관 2,720·우체통 4,080·전망대 5,440·상점 6,800마리)과 공사 시간(회관 1분~상점 4시간)은 위임받아 채택한 초기값이며 운영 후 조정할 수 있다. 2026.09.15에 대상 인원 × 1인당 요구량 공식과 기존 보유분·이전 건물 누적 기록 인정을 없애고 고정 총액·각자 몫으로 바꿨다. 사람이 많을수록 각자 몫이 작아져 빨리 짓고, 주민이 하루 2시간 집중하면 15명 섬도 15일차(만 2주)에 상점까지 짓는다. 도서관의 주민별 누적 획득 기록은 초기화하지 않는다. 목표 변경 시 이미 모은 몫을 이어서 셀지, 일일 퀘스트 보상을 각자 몫에 넣을지는 추가 결정이 필요하다.","link":"feature-inventory.html?tab=growth","date":"2026.09.14","source":"사용자 결정 · 현재 정책 2026-09-14"},
