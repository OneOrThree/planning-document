...
(() => {
  ...
  const entries = [
    {"id":"policy-board-notice-comment","status":"confirmed","title":"게시판 공지·댓글의 작성·삭제 권한과 글자 상한을 확정한다","body":"공지 작성·수정·삭제는 방장만 한다. 방장 외 주민에게 공지 권한을 열어주던 이전 결정(D9)은 이 결정으로 대체한다. 길이 상한은 공지 제목 100자를 유지하고 공지 본문 5000자·댓글 500자를 새로 정하며, UTF-16 단위 기준으로 넘으면 거절한다. 공지를 삭제하면 그 공지에 달린 댓글도 함께 삭제한다. 댓글 작성은 주민만 하고(방문자는 읽기 전용, 기존 결정) 댓글 삭제는 작성자 본인과 방장이 한다. 주민이 탈퇴하면 그 주민이 쓴 댓글 원문을 삭제하며 익명으로 남기지 않아 개인정보 파기 범위를 단순하게 유지한다. 백엔드 정책 BQ02·BQ03 미결로 잠겨 있던 게시판 쓰기는 이 결정으로 풀리지만, 이 기록 자체는 기획 확정이며 구현 완료를 뜻하지 않는다.","link":"feature-inventory.html?mode=detail#feature-G02","date":"2026.09.25","source":"GROMO-2136 · 재영님 결정"},
    ...
    {"id":"auth-provider-platform-matrix","status":"confirmed","title":"로그인 제공자는 플랫폼·배포 구성별로 나눈다","body":"iOS 카카오 구성은 카카오·Apple·Google·게스트, iOS LINE 구성은 LINE·Apple·Google·게스트를 제공한다. Android 카카오 구성은 카카오·Google·게스트, Android LINE 구성은 LINE·Google·게스트를 제공한다. Android에는 Apple 로그인을 노출하지 않는다. 회원 계정에는 이 중 한 가지 로그인 수단만 연결하고 제공자 고유 ID만 저장한다.","link":"privacy-data-inventory-2026-09-21.md","date":"2026.09.21","source":"GROMO-1869 완료조건 1 · 사용자 결정"},
    {"id":"withdrawal-data-disposition","status":"confirmed","title":"탈퇴 데이터는 삭제·연결 해제·기간 보존으로 나눠 처리한다","body":"탈퇴 시 세션·인증 수단·푸시 토큰 연결을 즉시 끊고, 운영 데이터베이스의 직접 식별정보와 앱별 사용 원본은 탈퇴 처리 중 삭제하거나 사용자 연결을 해제한다. 7일은 운영 데이터의 대기기간이 아니라 데이터베이스 백업의 최대 만료기간이다. 공동 콘텐츠는 작성자 연결을 끊어 유지하고, 로그와 분석 데이터는 항목별 보존기간이 끝나면 삭제한다. 실제 삭제 배선과 외부 도구 설정을 검증하기 전에는 구현 완료로 표시하지 않는다.","link":"withdrawal-data-disposition-2026-09-21.md","date":"2026.09.22","source":"GROMO-1869 완료조건 3 · 사용자 결정"},
    {"id":"onboarding-privacy-consent","status":"confirmed","title":"가입 동의와 섬 기록 공개 확인을 분리한다","body":"만 14세 이상 확인, 이용약관, 필수 개인정보 처리, 섬 활동·기록 공개 확인을 한 줄의 통합 체크박스로만 받지 않는다. 첫 섬 생성·가입 직전에 집중 중 할 일·진행 시간과 도서관의 집중·하루 총 스크린타임 기록이 같은 섬 주민 전체에게 공개되고 숨김 설정이 없다는 사실을 별도로 확인받는다. 다른 섬 방문자는 도서관과 집중 중 정보를 볼 수 없지만 게시판 퀘스트의 주민별 달성률은 볼 수 있다. 스크린타임·앱별 사용 시간은 선택 동의와 OS 권한을 거치며, 거부해도 집중 기능을 계속 쓴다. 앱별 사용 시간과 차단·허용 앱 목록은 다른 사용자에게 공개하지 않는다.","link":"onboarding-privacy-consent-2026-09-21.md","date":"2026.09.21","source":"GROMO-1869 완료조건 2 · 사용자 결정"},
    {"id":"policy-user-blocking","status":"confirmed","title":"사용자 차단은 법적·플랫폼 정책 준수 리스크 때문에 2.0 선행 범위로 앞당긴다","body":"기존 결정의 “차단 기능은 2.0.0에서 제공하지 않는다”를 폐기한다. 친구 편지처럼 사용자 간 콘텐츠와 1:1 상호작용을 제공하면서 차단 진입점이 없으면 앱 심사·플랫폼 정책 준수 리스크가 있으므로, 후순위였던 사용자 차단을 2.0 선행 구현으로 앞당긴다. 친구 프로필과 받은 편지 상세에서 확인 뒤 차단하고, 설정에서 내가 차단한 사용자 목록을 조회·해제한다. 차단은 blocker 관점의 표시 필터로 친구 목록·친구 검색·받은 편지에서 상대를 숨기며 친구 관계와 편지 원문을 삭제하지 않는다. 서버 계약은 `GET·POST /blocks`, `DELETE /blocks/{blockedUserId}`이고 중복 차단·해제는 멱등하게 처리한다. 이 결정은 법률 자문 결과를 뜻하지 않으며, 현재 근거는 앱스토어 심사와 UGC 플랫폼 정책이다.","link":"feature-inventory.html?mode=detail#feature-I08","date":"2026.09.22","source":"GROMO-1975 · 플랫폼 UGC 정책"},
    {"id":"policy-golden-fish","status":"confirmed","title":"같이 집중할수록 황금 물고기가 자주 나타나고, 다 같이 낚는다","body":"같이 집중해도 혼자일 때와 보상이 같던 문제를 황금 물고기로 푼다. 같은 섬에서 2명 이상 같이 집중할 때 섬마다 1분에 한 번 추첨해, 황금 물고기가 나타나면 함께 낚은 주민 모두의 휴대폰에서 여러 고양이가 힘을 모아 낚아 올리는 약 3초 컷신을 재생하고 섬 잔액에 50마리를 한 번 더한다. 확률은 추첨 순간 같은 섬에서 집중 중인 주민 수(휴식 제외·최대 5명)로만 정하며 1분당 2명 0.4%, 3명 1.2%, 4명 2.4%, 5명 이상 4%다. 혼자 집중할 때는 나타나지 않는다. 480마리 상한과 별도로 지급하고 상한을 채운 주민도 인원에 넣는다. 50마리는 함께 낚은 주민의 누적 기록·건설 각자 몫에 인원수로 나눠(내림) 더한다. 황금 여부는 서버가 정한다. 혼자 집중할 때의 기본 보상은 1분당 1마리 그대로다.","link":"feature-inventory.html?mode=detail#feature-D22","date":"2026.09.19","source":"GROMO-1921 · 사용자 결정"},
    ...
    {"id":"policy-focus-fishing-island","status":"confirmed","title":"낚시섬의 원하는 곳에서 집중하고 전용 모닥불 화면에서 쉰다","body":"목표 시간 없이 할 일을 적고 카운트업으로 집중한다. 기본 뗏목으로 낚시섬에 도착한 뒤 원하는 곳을 눌러 자리를 정하며 앱이 정한 자리 목록은 없다. 핀치 확대는 내 자리, 축소는 지금 집중 중인 주민 전체다. 휴식하기를 누른 순간부터 휴식으로 세고 전용 모닥불 화면에서 식빵을 굽는다. 재개하면 기존 낚시 자리로 돌아간다. 집중 화면은 위 가운데에 할 일과 이번 집중 경과 시간, 아래에 이모티콘·휴식하기·집중 종료를 두고 현재 음악은 표시하지 않는다. 음악은 낚시섬의 축음기를 눌러 바꾸며 고양이는 걸어가지 않는다. 축소하면 다른 주민은 이름만 남기고, 이모티콘은 3초 동안 그 고양이의 과목·경과 시간 표시를 대신한다. 물고기는 유효 집중 60초마다 섬 잔액에 바로 넣는다. 휴식 중 앱을 꺼도 휴식은 이어지고, 1시간 방치하면 집중을 자동 종료해 정상 완료로 기록한다.","link":"feature-inventory.html?mode=detail&group=focus","date":"2026.09.17","source":"사용자 결정 · 현재 정책 2026-09-17"},
    {"id":"policy-library","status":"confirmed","title":"집중 기록은 도서관에서 같은 섬 전체 공개로 본다","body":"마을회관 통계를 도서관으로 옮긴다. 도서관 완공 후 같은 섬 모든 주민의 집중·스크린타임 기록을 일·주·월 단위로 보고, 주민별 누적 물고기 획득 기록도 여기서 본다. 같은 섬 주민 전체 공개로 고정해 설정을 두지 않는다. 2026.09.19 GROMO-1904 결정에 따라 다른 섬 방문자는 도서관을 이용하거나 기록을 볼 수 없다. 완공 전 이전 기록은 볼 수 없고 데이터 없음·권한 없음·실제 0을 구분한다.","link":"feature-inventory.html?mode=detail#feature-H01","date":"2026.09.14","source":"사용자 결정 · 현재 정책 2026-09-14"},
    {"id":"policy-join-capacity","status":"confirmed","title":"가입 방식과 정원은 방장이 정하고 신청은 취소할 수 있다","body":"섬을 만들 때 방장이 가입 방식(승인 필요·승인 불필요)과 정원을 정하고, 정원은 마을회관에서 수정한다. 승인 대기 중인 가입 신청은 취소할 수 있고 다시 신청할 수 있다. 복수 소속을 허용하며 새로 가입해도 기존 섬에서 탈퇴하지 않는다. 2026.09.15에 정원 기준을 더 정했다. 정원은 1~15명이고 따로 정하지 않으면 15명이다. 방장을 포함한 현재 주민만 세며 승인 대기 신청은 세지 않는다. 현재 주민 수보다 작게 줄일 수 없고, 가득 찬 섬은 즉시 가입·초대 코드 입장·새 신청을 막는다. 대기 중인 신청은 승인할 때 정원을 다시 확인한다. 2026.09.16 추가 결정(GROMO-1867): 모든 사용자는 메인 섬을 하나 가진다. 첫 섬을 만들거나 가입하면 그 섬이 메인 섬이 되고, 내 뗏목의 내 정보에서 내가 속한 섬 중 하나로 바꿀 수 있다. 메인 섬에서 탈퇴하거나 강퇴되면 남은 섬 중 가장 최근에 가입한 섬이 메인 섬이 되고 알림을 보낸다. 메인 섬 이름은 친구 목록과 친구 프로필에만 표시하고 친구가 아닌 사용자의 검색 결과에는 표시하지 않으며, 물고기·집중 기록 적립과 랭킹에 영향을 주지 않는다. 앱을 켜면 마지막으로 접속한 섬에서 시작한다. 2026.09.20: 강퇴로 소속 섬이 하나도 남지 않은 사용자도 첫 소속 선택 화면으로 간다.","link":"feature-inventory.html?mode=detail#feature-B10","date":"2026.09.14","source":"사용자 결정 · 현재 정책 2026-09-14"},
