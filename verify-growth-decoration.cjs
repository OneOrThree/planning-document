...
(async()=>{
  ...
  check(feature('E03').decision==='chosen'&&feature('E03').action.includes('고정 총액')&&feature('E03').action.includes('건설하기')&&feature('L14').decision==='proposal','건설 차감은 고정 총액, 테마 가격은 미정 유지');
  ...
  check(feature('E14').note.includes('강아지')&&feature('E14').note.includes('총액 6,800마리'),'강아지 상점 NPC·마지막 상점 고정 총액');
  check(feature('D20').action.includes('원하는 곳')&&!feature('D20').action.includes('모든 배')&&feature('D20').note.includes('자리 목록이나 번호는 없다'),'낚시섬 자유 자리/지금 집중 중인 주민');
  check(feature('D14').action.includes('전용 모닥불 화면')&&feature('D14').note.includes('같은 자리·같은 세션')&&!feature('D14').action.includes('배 위 낚시'),'전용 휴식 화면 후 같은 자리·세션 복귀');
  ...
  check(feature('F13').note.includes('완료 도장')&&feature('F13').note.includes('건설하기')&&feature('F13').note.includes('일일 초기화하지 않')&&feature('D22').note.includes('1분(60초)')&&feature('D22').note.includes('하루 최대 480마리'),'건물 건설 퀘스트·1분 1마리·하루 최대 480마리 기준');
  ...
  check(feature('D12').note.includes('이번 휴식 경과 시간')&&feature('D07').note.includes('보유 음원')&&feature('B06').note.includes('공개 섬 둘러보기에는 초대 코드 입력을 두지 않아요'),'휴식 타이머·현재 음악 선택·초대 코드 진입 위치');
